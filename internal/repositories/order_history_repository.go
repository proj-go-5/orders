...
func (h OrderHistoryRepository) GetHistoryByOrderId(ctx context.Context, orderID int) ([]models.OrderHistory, error) {
	var history []models.OrderHistory
	result := h.connection.WithContext(ctx).Find(&history, "order_id = ?", orderID)
	return history, result.Error
}

func (h OrderHistoryRepository) AddHistoryRecord(ctx context.Context, record *models.OrderHistory) error {
